...
@bp.route("/kpis/quick-create", methods=["GET", "POST"])
@login_required
@organization_required
@permission_required("can_manage_kpis")
def quick_create_kpi():
    """Quick KPI creation with auto-hierarchy creation"""
    org_id = session.get("organization_id")
    ...
    # Check prerequisites
    if not value_types:
        flash("⚠️ Please create at least one Value Type first.", "warning")
        return redirect(url_for("organization_admin.create_value_type"))

    if not governance_bodies:
        flash("⚠️ Please create at least one Governance Body first.", "warning")
        return redirect(url_for("organization_admin.create_governance_body"))
    ...
    if request.method == "POST":
        ...
        system_name = request.form.get("system_name", "General").strip()
        kpi_description = request.form.get("kpi_description", "").strip()
        selected_vt_ids = request.form.getlist("value_type_ids")
        selected_gb_ids = request.form.getlist("governance_body_ids")

        # Validation
        if not kpi_name:
            flash("KPI name is required", "danger")
            return render_template(
                "organization_admin/quick_create_kpi.html",
                value_types=value_types,
                governance_bodies=governance_bodies,
        ...
        if not selected_vt_ids:
            flash("Please select at least one value type", "danger")
            return render_template(
                "organization_admin/quick_create_kpi.html",
                value_types=value_types,
                governance_bodies=governance_bodies,
        ...
        # 1. Space
        space = Space.query.filter_by(organization_id=org_id, name=space_name).first()
        if not space:
            space = Space(organization_id=org_id, name=space_name or "General", display_order=0)
            db.session.add(space)
            db.session.flush()

        # 2. Challenge
        challenge = Challenge.query.filter_by(organization_id=org_id, name=challenge_name).first()
        if not challenge:
            challenge = Challenge(
                organization_id=org_id, space_id=space.id, name=challenge_name or "General", display_order=0
            )
            db.session.add(challenge)
            db.session.flush()

        # 3. Initiative
        initiative = Initiative.query.filter_by(organization_id=org_id, name=initiative_name).first()
        if not initiative:
            initiative = Initiative(organization_id=org_id, name=initiative_name or "General", description="")
            db.session.add(initiative)
            db.session.flush()

        # 4. Link initiative to challenge (if not already linked)
        init_challenge_link = ChallengeInitiativeLink.query.filter_by(
            challenge_id=challenge.id, initiative_id=initiative.id
        ).first()
        if not init_challenge_link:
            init_challenge_link = ChallengeInitiativeLink(
                challenge_id=challenge.id, initiative_id=initiative.id, display_order=0
            )
            db.session.add(init_challenge_link)
            db.session.flush()

        # 5. System
        system = System.query.filter_by(organization_id=org_id, name=system_name).first()
        if not system:
            system = System(organization_id=org_id, name=system_name, description="")
            db.session.add(system)
            db.session.flush()

        # 6. Link system to initiative (if not already linked)
        init_system_link = InitiativeSystemLink.query.filter_by(
            initiative_id=initiative.id, system_id=system.id
        ).first()
        if not init_system_link:
            init_system_link = InitiativeSystemLink(
                initiative_id=initiative.id, system_id=system.id, display_order=0
            )
            db.session.add(init_system_link)
            db.session.flush()

        # 7. Create KPI
        kpi = KPI(initiative_system_link_id=init_system_link.id, name=kpi_name, description=kpi_description or "")
        db.session.add(kpi)
        db.session.flush()

        # 8. Link governance bodies
        for gb_id in selected_gb_ids:
            gb_link = KPIGovernanceBodyLink(kpi_id=kpi.id, governance_body_id=int(gb_id))
            db.session.add(gb_link)

        # 9. Link value types with smart defaults
        for vt_id in selected_vt_ids:
            vt = ValueType.query.get(int(vt_id))
            config = KPIValueTypeConfig(
                kpi_id=kpi.id,
                value_type_id=int(vt_id),
                display_order=0,
                color_positive="#28a745",
                color_zero="#6c757d",
                color_negative="#dc3545",
                display_scale="default",
                display_decimals=vt.decimal_places if vt.is_numeric() else None,
                calculation_type="manual",
            )
            db.session.add(config)
        ...
        db.session.commit()
        flash(f"✅ KPI {kpi.name} created successfully with full hierarchy!", "success")
        return redirect(url_for("workspace.index", auto_edit=1))
