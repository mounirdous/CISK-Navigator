...
from app.models import (
    ValueType, Space, Challenge, Initiative, System, KPI,
...
class YAMLImportService:
        ...
        try:
            ...
            if 'value_types' in data:
                for vt_data in data['value_types']:
                    try:
                        vt = YAMLImportService._create_value_type(
                            vt_data, organization_id
                        )
                        if not dry_run:
                            db.session.add(vt)
    ...
    @staticmethod
    def _create_space_and_children(space_data, organization_id, value_type_map,
                                     initiative_map, system_map, dry_run):
    ...
    @staticmethod
    def _create_challenge_and_children(challenge_data, space_id, organization_id,
                                         value_type_map, initiative_map, system_map, dry_run):
    ...
    @staticmethod
    def _create_system_and_kpis(system_data, initiative_id, organization_id,
                                  value_type_map, system_map, dry_run):
        ...
        link = InitiativeSystemLink(
            ...
            system_id=system.id,
            display_order=system_data.get('display_order', 0)
        )
        if not dry_run:
            db.session.add(link)
            db.session.flush()
