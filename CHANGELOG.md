...
### Changed
- **Initiative-review sequence export** — large overhaul to make the stitched multi-initiative HTML behave like a self-contained reader rather than a long scroll-document with broken nav. Every change below is in the **exported file only**; the live initiative form is untouched.
    - **Each initiative is its own paginated "page"** (one visible at a time) instead of a vertical scroll. The shim wraps each rendered initiative in `<div class="snapshot-page" data-section-id="initiative-N">`; CSS shows only `.snapshot-page.active`. Prev / Next / Dot / ArrowLeft / ArrowRight swap which page is active via `_showPage(target)`.
    - **Per-page render now carries the correct `nav_pos`.** Earlier every rendered initiative got `nav_pos=0` because all pages shared the export request's QUERY_STRING, so pages 2..N had a broken nav bar (prev disabled, next pointing at page 1's neighbour, "current" dot on the wrong cell). The service now pushes a fresh `current_app.request_context(env)` per render with `nav={…}&nav_pos={pos}`. **Crucially**, the new context inherits `g._login_user` and the outer session dict from the export request, otherwise `@login_required` redirected to `/auth/login` and pages 2..N silently became 385-byte login-redirect stubs. Auth-carry verified end-to-end (24-init export, 2.8MB, all 24 sections render, 0 redirects).
    - **Single source of truth for prev/next**: `_navSequence()` builds the active navigation order (focus-sorted when focus mode is on, DOM order otherwise), and prev/next/arrow walk it consistently regardless of which page's button was clicked. Earlier the live JS rewrote page 1's prev/next hrefs to walk focus-sorted order while pages 2..N kept DOM-order hrefs, so the user got "page 1 walks focus, page 2 walks DOM" inconsistencies.
    - **Position counter ("3 / 24") syncs with the highlighted bullet.** Counter is baked at render time as DOM position; in focus mode the bullets reorder by impact. The shim now rewrites `.review-nav-position` text content to the active page's index in the effective sequence, on every page swap and focus-mode toggle, so counter and bullet always match.
    - **Focus mode (group dots by impact)** is reapplied to the active page on every page swap. `_reapplyFocus()` reads the org's `IMPACT_SCALE` icons (★ / ★★ / ★★★ in green/orange/red for INFORMATION SECURITY) and inserts them as group separators in the active page's `.review-nav-dots`. Earlier the live focus-mode JS only operated on page 1's nav bar (because `document.querySelector('.review-nav-dots')` returns the first match); switching pages with focus on lost the visual.
    - **Form / Execution tab switching** scoped to the active page. `window.switchTab` and `window.setReviewTab` overridden via `setTimeout(0)` after the live script defines them; `_applyTab(tab)` toggles the `.active` / `.show` classes on the active page's `#form-tab` / `#execution-tab` / `#form-tab-panel` / `#execution-tab-panel`. The user's tab choice is remembered and re-applied on every page swap so navigating preserves Form-vs-Execution preference.
    - **Detail-mode (xmas tree) forced to "max"** on every per-page container via `[id="initReviewContainer"]` attribute selector (querySelectorAll across stitched duplicates), and the toggle button (`#detailModeBtn`) hidden via CSS. Earlier the live `applyDetailMode` only updated page 1's container, so pages 2..N rendered at template-default detail.
    - **`navToPos` / `navToDot` neutralised** as no-ops post-DOMContentLoaded so even if my `stopImmediatePropagation` misses some keyboard event, the live nav functions can't navigate to the live URL.
    - **All live-page links deactivated.** `<base href>` no longer injected (right-click "Copy link" no longer leaks the live URL). A capture-phase click handler `preventDefault`s every `<a>` click except intra-doc `#hash` anchors, and CSS sets `cursor: default` on those non-interactive anchors so the reader sees they're inert. The snapshot-controlled buttons (prev/next/dot/focus/tabs) are unaffected because they `stopImmediatePropagation` first.
    - **Snapshot-only chrome hidden in CSS**: `× Exit`, Definition/Execution/Timeline tab toggle in the header (links to live timeline page), `Back to Workspace`, the `+ New` action button, and the `Export HTML` icon (we *are* the export).
    - **Export filename uses the org name** (`InitiativeReview_<orgname>.html`) for sequence exports — matches the workspace tree export's filename shape.

- **Live initiative-form Export-HTML button styling** matches the Edit button — same `btn btn-outline-light` class, icon + short "Export" text, same shape and size. Replaces the previous tiny icon-only button that looked mismatched next to Edit.
...
### Added
- **Multi-VT KPI integration test suite** (`tests/integration/test_multi_vt_kpi.py`, 11 tests, all green). Phase 3 of the multi-VT story — there were zero tests covering the assumption "every code path iterates `value_type_configs`". The new suite pins: schema can hold N configs per KPI; create form POST with two `value_type_ids` makes two configs; edit form adds new VTs / removes unused ones / blocks removal of VTs with contributions until `confirm_remove_vt_ids` names them / cascade-deletes contributions on confirmed removal / refuses zero-VT submissions; KPI dashboard route emits per-(KPI × VT) rows and the "tracked rows" subtitle disappears when cell count == KPI count; geography `api_map_kpis` returns `value_types[]` per feature.

### Fixed
- **KPI dashboard hid the value-type label on rows with no consensus value yet**. The `value_type_name` was rendered inside the `{% if k.formatted_value %}` branch, so multi-VT cells with no contributions showed only `—` with no indication of which VT the row belonged to. Pulled the VT-name span out of the value-truthy branch in `kpi_dashboard.html` so it always renders when known. Caught by the new dashboard test.
...
### Fixed
- Added `before_request` guard that clears `session["organization_id"]` (plus `_name` / `_logo`) when the org no longer exists, so users whose current workspace was hard-deleted are returned to a clean state instead of carrying a ghost org context across requests

## [7.18.2] - 2026-04-28

### Fixed
- Fixed `IntegrityError: fk_audit_logs_organization_id` when the user's session carried a stale `organization_id` (e.g. org was hard-deleted). `AuditService.log_action` now validates the session org exists before attaching it to audit log inserts, falling back to `NULL`

## [7.18.1] - 2026-04-28

### Fixed
- Fixed `UnboundLocalError: cannot access local variable '_org'` on `/workspace/data` when `impact_scale` was empty — `_org` is now resolved unconditionally before the impact-importance block
