...
def extract_metadata(yaml_data: Dict) -> Dict:
    ...
    default_colors = {"challenge": "#f0d24f", "initiative": "#8fd0ff", "system": "#1d4ed8", "kpi": "#22c55e"}
...
def _create_base_structure(source_title: str) -> Dict:
    ...
    return {
        "meta": {"generated": datetime.now().isoformat(), "sourceDeck": source_title},
        "challengeGroups": [],
        "subChallenges": [],
        "initiatives": [],
        "systems": [],
...
def convert_enhanced_format(yaml_data: Dict) -> Dict:
    ...
    for sys in yaml_data.get("systems", []):
        sys_obj = {"id": sys["id"], "text": sys.get("text", ""), "title": sys.get("title", ""), "initiatives": []}
        ...
        for link in sys.get("initiatives", []):
            sys_obj["initiatives"].append({"initiativeId": link["initiative_id"], "weight": link.get("weight", 5)})

        js_data["systems"].append(sys_obj)

    # Convert KPIs (with system links and weights)
    for kpi in yaml_data.get("kpis", []):
        kpi_obj = {"id": kpi["id"], "text": kpi["text"], "systems": []}

        # Add system links with weights
        for link in kpi.get("systems", []):
            kpi_obj["systems"].append({"systemId": link["system_id"], "weight": link.get("weight", 5)})
...
def index():
    ...
    try:
        ...
        return render_template("navigator_enhanced.html", data=js_data, app_version=APP_VERSION, **metadata)
...
def upload():
    ...
    try:
        ...
        return render_template(
            "navigator_enhanced.html", data=js_data, app_version=APP_VERSION, upload_stats=stats, **metadata
        )
    except Exception as e:
...
def generate_standalone():
    ...
    try:
        ...
        response.headers["Content-Disposition"] = (
            f'attachment; filename="cisk_navigator_enhanced_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html"'
        )
